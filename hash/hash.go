...
func CrearHash[K comparable, V any]() Diccionario[K, V] {
	...
	diccionario.datos = make([]celda[K, V], 10)
	return diccionario
}

// obtenerPosicion devuelve la posición que le corresponde a la clave pasada luego de aplicarle la función de hash
...
// buscarClave recorre la tabla de hash y devuelve la posicion en donde se encuentra la clave pasada
...
func (diccionario *hashCerrado[K, V]) Borrar(clave K) V {
	...
	if diccionario.calcularCarga() <= CRITERIO_ACHICAR {
		diccionario.redimensionar(len(diccionario.datos) / 2)
	}
	pos := diccionario.obtenerPosicion(clave, len(diccionario.datos))
	buscarClave := diccionario.buscarClave(pos, clave)
